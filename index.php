<?php
session_start();

if(isset($_SESSION["game_id"])){
    header("location: game.php");
    exit();
}

?>
<!doctype html>
<html lang="hu">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Tic-Tac-Toe játék</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
    <style>
        .card{
            margin-left: 20px;
            margin-right: 20px;
            margin-top: 20px;
        }
        body{
            background-image: url("https://www.xmple.com/wallpaper/blue-graph-paper-grid-purple-1920x1080-c2-e6e6fa-5f9ea0-l2-3-93-a-0-f-20.svg");
        }
    </style>
</head>
<body>
<nav class="navbar navbar-light bg-light">
    <div class="container-fluid">
        <span class="navbar-brand mb-0 h1">Tic-Tac-Toe játék</span>
    </div>
</nav>
<div class="card">
    <div class="card-header">
        Játékinditó
    </div>
    <div class="card-body">
        <form action="newgame.php">
            <div class="mb-3">
                <label for="playername" class="form-label">Játékos neve:</label>
                <input type="text" class="form-control" id="playername" name="playername">
            </div>
            <button type="submit" class="btn btn-primary">Új játék inditása</button>
        </form>
    </div>
</div>
<div class="card">
    <div class="card-header">
        Eddigi eredmények
    </div>
    <div class="card-body">
        <div class="table-responsive">
        <table class="table">
            <thead>
            <tr>
                <th scope="col">Játék ID</th>
                <th scope="col">Játékos neve</th>
                <th scope="col">Nyertes</th>
                <th scope="col"></th>
            </tr>
            </thead>
            <tbody>
            <?php
                require_once "dbconnector.php";
                $stmt = $conn->prepare("SELECT * FROM `games` ORDER BY `id` desc");
                $stmt->execute();
                $result = $stmt->get_result();
                $html = "";
                while ($row = $result->fetch_assoc()) {
                    $html .= <<<HTML
                    <tr>
                      <th scope="row">{$row["game_id"]}</th>
                      <td>{$row["player_name"]}</td>
                      <td>{$row["winner"]}</td>
                      <td><a href="watch.php?game={$row["game_id"]}" class="btn btn-primary">Megtekintés</a></td>
                    </tr>
HTML;
                }
                echo $html;
            ?>
            </tbody>
        </table>
        </div>
    </div>
</div>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
</body>
</html>
